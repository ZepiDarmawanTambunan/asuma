...
import { getFirestore, collection, onSnapshot, deleteDoc, doc, query, where, limit, orderBy, getDoc } from 'firebase/firestore';
...
function SuratMasuk() {
  ...
  // SURAT LIST COUNT TOTAL PAGE FOR PAGINATE
  useEffect(() => {
    const querySurat = query(suratRef, 
      where('jenisSurat', '==', 'surat-masuk'),
      where('tujuan', '==', bagianId),
    );
  ...
  const fetchBagian = async () => {
    try {
      ...
      const bagianId = bagianList.filter((e) => e.value === docSnapUser.data().bagianId)[0]?.value;
      setBagianId(bagianId);
    } catch (error) {
      alert('error :'+error);
    }
  }
  ...
  // HANDLE CONVERT ASALBAG ID TO ASAL NAMA
    ...
    <tbody>
      {
        ...
        ? currentItems.map((surat) => (
          ...
          >
            ...
            <td className="py-2 px-4 border-b border-gray-300 text-center">{convertAsalBagianIdToAsalNama(surat.asal)}</td>
        ...
        <ModalSuratMasuk isOpen={isModalOpen} onClose={closeModal} surat={editedSurat} bagianIdSurat={bagianId}/>
